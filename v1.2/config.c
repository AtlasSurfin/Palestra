...
Config load_conf(const char *filename){
    ...
    while(fgets(linea, sizeof(linea), file)){
        ...
        if(sscanf(linea, "%[^=]=%lf", chiave, &valore_dec) == 2){
            ...
            else if(strcmp(chiave, "P_SERV_MIN") == 0) conf.p_serv_min = (double)valore_dec;
            else if(strcmp(chiave, "P_SERV_MAX") == 0) conf.p_serv_max = (double)valore_dec;
            else if(strcmp(chiave, "N_REQUESTS") == 0) conf.n_requests = (int)valore_dec;
        }
    }
