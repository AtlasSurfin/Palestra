...
void save_stats(StatoPalestra *p, StatServizio *t, Config c){
    ...
    if(f == NULL){
        perror("[MANAGER] Errore apertura file CSV");
        return;
    }
    ...
    //Scrivo i dati
    fprintf(f, "%d;%d;%d;%.2f;%.2f\n", p->giorno_corrente, t->serviti_tot, t->non_serviti_tot, media, rapporto);
    fclose(f);
...
int main(){
    ...
    for(g = 0; g < conf.sim_duration; g++){
        ...
        while(min_trascorsi < 400){
            pause(); //Aspettiamo il tick del cronometro
        }

        //Calcolo atleti persi
        struct msqid_ds q_status;
        ...
        if(persi_oggi > conf.explode_threshold){
            strcpy(causa_chiusura, "EXPLODE");
            g++;
            break;
        }
        ...
        //Reset stats per giorno successivo
        sem_wait(&(palestra->mux_stats));
        for(int i = 0; i < NOF_SERVICES; i++){
            palestra->stats[i].serviti_oggi = 0;
            palestra->stats[i].non_serviti_oggi = 0;
